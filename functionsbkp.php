...
add_action('admin_post_export_members_filtered', 'export_members_filtered_csv');

function export_members_filtered_csv(){

    if (!current_user_can('manage_options')) {
        wp_die('Unauthorized');
    }

    if (!isset($_GET['membership_plan'])) {
        wp_die('Membership plan missing');
    }

    $membership_plan = intval($_GET['membership_plan']);
    ...
    if(!$query->have_posts()){
        wp_die('No members found');
    }

    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename=members-plan-'.$membership_plan.'.csv');
    ...
    while($query->have_posts()){
        ...
        // get linked user ID
        $user_id = get_post_meta($member_id,'afwum_member_user',true);
        ...
        fputcsv($output, array(
            ...
            $username,
            $email,
            get_post_meta($member_id,'af_member_plan',true),
            get_post_meta($member_id,'af_member_status',true),
            get_post_meta($member_id,'af_member_since',true),
            get_post_meta($member_id,'af_member_expiry',true),
            get_post_meta($member_id,'af_membership_granted',true),
            get_post_field('post_content',$member_id)
        ));
    }
...
add_action('restrict_manage_posts', function($post_type){
    ...
    if ($post_type != 'af_member') return;

    if(isset($_GET['membership_plan'])){

        $url = admin_url(
            'admin-post.php?action=export_members_filtered&membership_plan='
            . intval($_GET['membership_plan'])
        );

        echo '<a href="'.$url.'" class="button button-primary" style="margin-left:10px; float:right">Export This Membership</a>';
    }
    ...
    if (have_rows('featured_products')) {$i = 1;
        while (have_rows('featured_products')) {
            ...
            if ($i == 1) {
                $data .= '<div class="wp-block-columns is-layout-flex wp-container-4 wp-block-columns-is-layout-flex"> ';
            }
    ...
    $specific_product_id = [587, 591, 592]; // Change this to your actual product ID
    $product_found       = false;
...
*/
    ...
    if ($type == 'detail') {
        return $last_shipment_details = get_user_meta($user_id, 'last_shipment_details', true);
    }
...
*/
    ...
    if (strpos($_SERVER['REQUEST_URI'], 'pos') !== false) {
        ...
        remove_action('woocommerce_register_post', [af_wum()->registration, 'validate_membership_fields_data'], 10, 3);
    }
    }
    ...
    {
    $roles_to_delete = ['pms_subscription_plan_308', 'pms_subscription_plan_307', 'pos'];
        ...
        <?php
            ...
            function shipment_detail_page()
            {
                ...
                if (isset($_REQUEST['user_id']) && isset($_REQUEST['entry_id'])) {
                    ...
                    $product_name = FrmProEntriesController::get_field_value_shortcode(['field_id' => 112, 'entry' => $_REQUEST['entry_id']]);
                    $product_name = explode(',', $product_name);

                    $qty = FrmProEntriesController::get_field_value_shortcode(['field_id' => 113, 'entry' => $_REQUEST['entry_id']]);
                    $qty = explode(',', $qty);
...
';
            ...
            function frm_duplicate_entry_form($atts)
            {
                ...
                if ($entry_id) {
                    ...
                    global $wpdb;
                    $wpdb->update($wpdb->prefix . 'frm_items', ['user_id' => get_current_user_id()], ['id' => $new_entry_id]);
                    $wpdb->update($wpdb->prefix . 'frm_item_metas', ['meta_value' => get_current_user_id()], ['item_id' => $new_entry_id, 'field_id' => $user_id_field]);

                    $member_field_ids = [138, 187, 185];

                    foreach ($member_field_ids as $field_id) {
                        $wpdb->update(
                ...
                return FrmFormsController::get_form_shortcode(['id' => 13]);
            ...
            function bbloomer_shipment_content()
            {
                ...
                if (! isset($_GET['staging'])) {
                    ...
                    $member_field_ids = [138, 187, 185];
                    $placeholders     = implode(',', array_fill(0, count($member_field_ids), '%d'));

                    $results = $wpdb->get_results(
                        $wpdb->prepare(
                            "SELECT `item_id`, `field_id` FROM `{$wpdb->prefix}frm_item_metas`
         WHERE `meta_value` LIKE %s AND field_id IN ($placeholders) LIMIT 1",
                ...
                if ($field->id == 138) { // Replace YOUR_SELECT_FIELD_ID with the ID of your select field
                    ...
                    if ($query->have_posts()) {
                        while ($query->have_posts()) {
                            ...
                            $memberarr[$page_title][] = ['id' => $email, 'title' => get_the_title()];
                    ...
                    foreach ($memberarr as $category => $posts) {
                        $options[] = ['label' => $category, 'values' => ''];
                        foreach ($posts as $post) {

                            $options[] = ['label' => '&nbsp; &nbsp;' . $post['title'], 'value' => $post['id']];
                        }
            ...
            function my_generate_nonce_shortcode($atts)
            {
                ...
                $atts = shortcode_atts(['action' => 'switch_to_user'], $atts, 'generate_nonce');
                ...
                if (function_exists('current_user_switched')) {
                    ...
                    if ($switched_user) {
                        ...
                        if (isset($_REQUEST['task'])) {
                            ...
                            $product_fields = array(112, 191);
                            $product_name = '';

                            foreach ( $product_fields as $field_id ) {

                                $value = FrmProEntriesController::get_field_value_shortcode(array(
                                    'field_id' => $field_id,
                                    'entry'    => $entry_id
                                ));
                            ...
                            if ( empty($product_name) ) {
                                echo '<p>No products found in allocation.</p>';
                                return;
                            }

                            // Get quantities
                            $qty = FrmProEntriesController::get_field_value_shortcode(array(
                                'field_id' => 113,
                                'entry'    => $entry_id
                            ));

                            $product_names = array_map('trim', explode(',', $product_name));
                            $quantities    = array_map('trim', explode(',', $qty));
                            ...
                            // $product_name = FrmProEntriesController::get_field_value_shortcode(['field_id' => 112, 'entry' => $entry_id]);
                            // $product_name = explode(',', $product_name);

                            // $qty = FrmProEntriesController::get_field_value_shortcode(['field_id' => 113, 'entry' => $entry_id]);
                            // $qty = explode(',', $qty);
        ...
        WHERE ( ( meta_key='first_name' OR meta_key='last_name' )
            ...
            function allocation_data_users($entry_id, $form_id)
            {
                if ($form_id == 13) { //replace 5 with the id of the form
                    ...
                    $old_id    = $entry_id;
                    $rawemails = FrmProEntriesController::get_field_value_shortcode(['field_id' => 138, 'entry' => $entry_id]);
                    ...
                    foreach ($emails as $email) {
                        ...
                        if ($user) {
                            ...
                            $wpdb->update($wpdb->prefix . 'frm_items', ['user_id' => $user_id], ['id' => $new_entry_id]);
                            $wpdb->update($wpdb->prefix . 'frm_item_metas', ['meta_value' => $user_id], ['item_id' => $new_entry_id, 'field_id' => '122']);

                            $wpdb->update($wpdb->prefix . 'frm_item_metas', ['meta_value' => ''], ['item_id' => $new_entry_id, 'field_id' => '138']);
...
table.td, .thwecmf-hook-order-details tr, .thwecmf-hook-order-details th, .thwecmf-hook-order-details td {
    ...
    <?php
        ...
        function apply_excise_tax_on_state_change()
        {
            ...
            $user       = wp_get_current_user();
            $is_founder = in_array('founder_member', (array) $user->roles);
            ...
            if (empty($selected_state) || ! isset($state_tax_rates[$selected_state])) {
                wp_send_json_error(['message' => 'Invalid state selected.']);
            }
            ...
            $excise_tax = $is_founder ? 0 : $state_tax_rates[$selected_state];

            // Add excise tax fee to cart
            WC()->cart->add_fee('Excise Tax', $excise_tax, false);
            WC()->cart->calculate_totals();

            wp_send_json_success(['excise_tax' => $excise_tax]);
        }
        ...
        add_action('woocommerce_checkout_update_order_review', function ($post_data) {
            ...
            if (! empty($data['shipping_state'])) {
                $selected_state = sanitize_text_field($data['shipping_state']);

                // Save state in session
                WC()->session->set('selected_excise_state', $selected_state);

                // Check if user is founder_member
                $user       = wp_get_current_user();
                $is_founder = in_array('founder_member', (array) $user->roles);

                // If founder_member, set excise tax session value to 0
                if ($is_founder) {
                    WC()->session->set('excise_tax_amount', 0);
                }
            }
        });
        ...
        add_action('woocommerce_cart_calculate_fees', function ($cart) {
            ...
            if (! $selected_state || ! isset($state_tax_rates[$selected_state])) {
                return;
            }
        ...
        function enqueue_checkout_state_change_script()
        {
            if (is_checkout()) {
                ...
                wp_localize_script('custom-checkout-js', 'wc_checkout_params', [
        ...
        add_action('wp_enqueue_scripts', function () {
            wp_enqueue_script(
                'frm-limit-rows',
                get_stylesheet_directory_uri() . '/js/frm-limit.js',
                ['jquery'],
                time(),
                true
            );
    ...
    <?php
        ...
        add_filter('woocommerce_package_rates', 'adjust_shipping_for_founder_and_coupon', 20, 2);
        function adjust_shipping_for_founder_and_coupon($rates, $package)
        {
            $user       = wp_get_current_user();
            $is_founder = in_array('founder_member', (array) $user->roles);

            // Check if 'ship45' coupon is applied
            $has_ship_coupon = WC()->cart && in_array('ship45', WC()->cart->get_applied_coupons());

            foreach ($rates as $rate_id => $rate) {

                // Founder member: free shipping
                if ($is_founder) {
                ...
                // Coupon 'ship45' applied: force shipping to $45
                elseif ($has_ship_coupon) {
        ...
        // Founder Members: Set product prices to 0 only on cart and checkout

        add_filter('woocommerce_product_get_price', 'fm_zero_price_everywhere', 999, 2);
        add_filter('woocommerce_product_get_regular_price', 'fm_zero_price_everywhere', 999, 2);
        add_filter('woocommerce_product_get_sale_price', 'fm_zero_price_everywhere', 999, 2);

        function fm_zero_price_everywhere($price, $product)
        {
            $user = wp_get_current_user();
            if (in_array('founder_member', (array) $user->roles)) {
                return 0;
            }
            return $price;
        }

        // Optional: update cart item price as well
        add_action('woocommerce_before_calculate_totals', 'fm_cart_items_zero_for_founder', 20, 1);
        function fm_cart_items_zero_for_founder($cart)
        {
            if (is_admin() && ! defined('DOING_AJAX')) {
                return;
            }
        ...
        // Hide subtotal in WooCommerce block checkout for founder_member
        ...
        // 4 November
        // Add City column in Users list

        add_filter('manage_users_columns', function ($columns) {

            $columns['billing_city'] = 'City';
        ...
        add_action('pre_get_users', function ($query) {
            ...
            if (isset($query->query_vars['orderby']) && $query->query_vars['orderby'] === 'billing_city') {

                $query->set('meta_key', 'billing_city');
            ...
            if (! empty($_GET['filter_city'])) {
                ...
                $meta_query = [
                    ...
                    [
                        ...
                        'value'   => sanitize_text_field($_GET['filter_city']),
        ...
        // Add City dropdown + inline Filter button
        ...
        add_action('restrict_manage_users', function ($which) {
            ...
            // Get unique cities from usermeta
    ...
    ");
            ...
            $selected_city = isset($_GET['filter_city']) ? sanitize_text_field($_GET['filter_city']) : '';

            // Inline dropdown + button with minimal spacing

            echo '<div style="display:inline-flex; align-items:center; gap:6px; margin-left:8px;">';

            echo '<select name="filter_city" style="min-width:150px;">';
        ...
        // function get_us_state_name($code) {
        //     $states = WC()->countries->get_states('US'); // WooCommerce state list
        ...
        // // Add State / Country column
        // add_filter('manage_users_columns', 'add_user_state_column');
        // function add_user_state_column($columns) {
        //     $columns['billing_state'] = __('State / Country');
        //     return $columns;
        // }

        // // Fill State / Country column
        // add_filter('manage_users_custom_column', 'show_user_state_column_data', 10, 3);
        // function show_user_state_column_data($value, $column_name, $user_id) {
        //     if ($column_name === 'billing_state') {

        //         $state   = get_user_meta($user_id, 'billing_state', true);     // NY
        //         $country = get_user_meta($user_id, 'billing_country', true);   // US

        //         // Convert US states
        //         if ($country === 'US' && $state) {
        //             $state = get_us_state_name($state); // NY → New York
        //         }

        //         // Convert country code
        //         if ($country) {
        //             $country = get_country_name($country); // US → United States
        //         }
        ...
        add_filter('frm_setup_new_fields_vars', 'frm_load_ul_members_by_plan', 20, 2);
        add_filter('frm_setup_new_fields_vars', 'frm_load_ul_members_by_plan', 20, 2);
        function frm_load_ul_members_by_plan($values, $field)
        {
            $wine_field_id      = 187;
            $community_field_id = 185;

            if ($field->id == $wine_field_id) {
                $values['options'] = ['' => '-- Select Wine Member --'] + frm_get_ul_members_by_plan('Wine Allocation');
            }

            if ($field->id == $community_field_id) {
                $values['options'] = ['' => '-- Select Community Member --'] + frm_get_ul_members_by_plan('Community Members');
            }

            return $values;
        }

        function frm_get_ul_members_by_plan($plan_name)
        {
            global $wpdb;
            ...
            $results = $wpdb->get_results(
                $wpdb->prepare(
                    ...
                    member.post_title AS member_name,
                    wp_users.user_email AS member_email
                FROM {$wpdb->posts} member

                INNER JOIN {$wpdb->postmeta} pm
                    ON pm.post_id = member.ID 
                    AND pm.meta_key = 'af_member_plan'

                INNER JOIN {$wpdb->posts} plan
                    ON plan.ID = pm.meta_value 
                    AND plan.post_title = %s

                INNER JOIN {$wpdb->postmeta} pm_user
                    ON pm_user.post_id = member.ID 
                    AND pm_user.meta_key = 'afwum_member_user'

                INNER JOIN {$wpdb->users} wp_users
                    ON wp_users.ID = pm_user.meta_value

                /* 👇 Add this join for status */
                INNER JOIN {$wpdb->postmeta} pm_status
                    ON pm_status.post_id = member.ID
                    AND pm_status.meta_key = 'af_member_status'
                    AND pm_status.meta_value = 'Active'

                WHERE member.post_type = 'af_member'
                  AND member.post_status = 'publish'

                ORDER BY member.post_title ASC
            ",
                    $plan_name
                )
            );
            ...
            foreach ($results as $row) {
                $options[$row->user_id] = $row->member_name . ' - ' . $row->member_email;
            }

            return $options;
        }

        // Allocation Functionality ( 6 and 12 Bottle Allocation)

        add_action('frm_after_create_entry', 'force_entry_name_to_total', 20, 2);
        add_action('frm_after_update_entry', 'force_entry_name_to_total', 20, 2);

        function force_entry_name_to_total($entry_id, $form_id){
            ...
            // CHANGE THIS to your TOTAL field ID
            $total = FrmEntryMeta::get_entry_meta_by_field($entry_id, 193, true);
        ...
        add_action('wp_footer', 'load_allocation_js');
        add_action('admin_footer', 'load_allocation_js');
        function load_allocation_js()
        {
        ?>
    <script>
        ...
        window.userAllocationData = <?php
                                        global $wpdb;

                                            $member_field_ids    = [187, 185]; // Wine and Community
                                            $allocation_field_id = 188;

                                            $entries = $wpdb->get_results("
        SELECT
            meta_member.meta_value AS raw_members,
            meta_alloc.meta_value AS allocation_type
        FROM wp_frm_item_metas meta_member
        JOIN wp_frm_item_metas meta_alloc
            ON meta_alloc.item_id = meta_member.item_id
        JOIN wp_frm_items items
            ON items.id = meta_member.item_id
        WHERE meta_member.field_id IN (187, 185)
          AND meta_alloc.field_id = 188
          AND YEAR(items.created_at) = YEAR(CURDATE())
        ");
                                            ...
                                            foreach ($entries as $row) {
                                                ...
                                                // unserialize multi-select field
                                                $members = maybe_unserialize($row->raw_members);

                                                if (is_string($members)) {
                                                    $members = [$members]; // convert single to array
                                                }
                                                ...
                                                foreach ($members as $member_string) {

                                                    // extract email from "Name - email"
                                                    if (! preg_match('/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i', $member_string, $m)) {
                                                        continue;
                                                    }

                                                    $email = strtolower(trim($m[0]));

                                                    if (! isset($allocations[$email])) {
                                                        $allocations[$email] = ['6' => 0, '12' => 0];
                                                    }

                                                    if ($row->allocation_type === '6 Bottle Allocation') {
                                                        $allocations[$email]['6']++;
                                                    }

                                                    if ($row->allocation_type === '12 Bottle Allocation') {
                                                        $allocations[$email]['12']++;
                                                    }
                                                }
                                            }
    ...
    function applyAllocationLimits() {

        const allocValue = document.querySelector('[name="item_meta[188]"]')?.value;
        if (!allocValue) return;

        let allocType = null;
        if (allocValue === "6 Bottle Allocation") allocType = "6";
        if (allocValue === "12 Bottle Allocation") allocType = "12";
        if (!allocType) return;

        // Pick correct multiselect
        let select = null;
        if (allocType === "6") select = document.querySelector('[name="item_meta[187][]"]');
        if (allocType === "12") select = document.querySelector('[name="item_meta[185][]"]');
        ...
        select.querySelectorAll("option").forEach(opt => {
            ...
            // Always reset first
            opt.disabled = false;
            opt.textContent = opt.textContent.replace(" (Already Allocated Twice)", "");

            // Ignore placeholder
            if (!opt.value || opt.value.startsWith("--")) return;

            const match = opt.value.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i);
            if (!match) return;

            const email = match[0].toLowerCase();
            const data = window.userAllocationData[email];
            if (!data) return;

            const count = data[allocType] || 0;

            if (count >= 2) {
                opt.disabled = true;
                opt.textContent += " (Already Allocated Twice)";
            }
        });
    }
    ...
    function getEmailFromOption(opt) {
        const match = opt.value.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i);
        return match ? match[0].toLowerCase() : null;
    }
    ...
    document.addEventListener("change", function(e) {
        if (e.target.name === "item_meta[188]") {
            applyAllocationLimits();
        }
    });
    document.addEventListener("DOMContentLoaded", applyAllocationLimits);

    document.addEventListener("DOMContentLoaded", function () {


        const allocationSelect = document.querySelector('select[name="item_meta[188]"]');

        function getMaxQuantity() {
            const allocationSelect = document.querySelector('select[name="item_meta[188]"]');
            if (!allocationSelect) return 0;

            if (allocationSelect.value === '6 Bottle Allocation') return 6;
            if (allocationSelect.value === '12 Bottle Allocation') return 12;
            return 0;
        }
        ...
        function enforceQuantityLimit() {
            ...
            const rows = document.querySelectorAll('.frm_repeat_sec');
            let runningTotal = 0;
            ...
            rows.forEach(row => {
                ...
                // Remaining bottles BEFORE this row
                const remaining = maxQty - runningTotal;
                ...
                // Set max attribute
                qtyInput.max = remaining >= 0 ? remaining : 0;

                // Disable row if no bottles left
                if (remaining <= 0) {
                    qtyInput.value = 0;
            ...
            // Hide Add button if total reached
            document.querySelectorAll('.frm_add_form_row').forEach(btn => {
                btn.style.display = (runningTotal >= maxQty) ? 'none' : '';
            });
        }
        ...
        /* =============================
           FILTER MEMBERS
        ============================= */
        function getMemberSelectByAllocation(allocation) {
            if (allocation === '6 Bottle Allocation') {
                return document.querySelector('select[name="item_meta[187][]"]');
            }

            if (allocation === '12 Bottle Allocation') {
                return document.querySelector('select[name="item_meta[185][]"]');
            }

            // Default allocation
            return document.querySelector('select[name="item_meta[138]"]');
        }

        function filterMembers() {
            if (!allocationSelect) return;

            const selected = allocationSelect.value;
            const memberSelect = getMemberSelectByAllocation(selected);
            ...
            if (!selected || selected === "Choose Allocation") return;

            if (selected === '6 Bottle Allocation') {
                options.forEach(opt => {
                    if (!opt.textContent.toLowerCase().includes('wine')) {
                        opt.hidden = true;
                        opt.disabled = true;
                    }
                });
            ...
            if (selected === '12 Bottle Allocation') {
                options.forEach(opt => {
                    if (!opt.textContent.toLowerCase().includes('community')) {
                        opt.hidden = true;
                        opt.disabled = true;
                    }
                });
            ...
            [...memberSelect.selectedOptions].forEach(opt => {
        ...
        function getRows() {
            return document.querySelectorAll('.frm_repeat_sec');
        }
        ...
        function updateTotals() {
            setTimeout(function() {
                ...
                rows.forEach(row => {
                    ...
                    // ✅ Get quantity field (113)
                    const qtyInput = row.querySelector('input[name*="[113]"]');
                    const qty = qtyInput ? parseFloat(qtyInput.value) || 0 : 0;

                    // ✅ Get product select (191)
                    const productSelect = row.querySelector('select[name*="[191]"]');
                    ...
                    if (productSelect && productSelect.selectedIndex >= 0) {
                        ...
                        if (selectedOption && selectedOption.dataset.frmprice) {
                            price = parseFloat(selectedOption.dataset.frmprice) || 0;
                        }
                    }
                    ...
                    const formattedTotal = row.querySelector('.frm_total_formatted');
                        if (formattedTotal) {
                            formattedTotal.textContent = '$' + rowTotal.toFixed(2);
                        }

                    const display = document.getElementById('custom_subtotal');
                    if (display) {
                ...
                const storageField = document.querySelector('input[name="item_meta[193]"]');// CHANGE ID
                if (storageField) {
                    storageField.value = grandTotal.toFixed(2);
                }
        ...
        document.body.addEventListener('click', function(e){
            if (e.target.closest('.frm_add_form_row') ) {
                // console.log("click working");

                setTimeout(() => {

                    // if (allocationSelect.value !== "Choose Allocation") {


                    //     document
                    //         .querySelectorAll('.frm_field_112_container')
                    //         .forEach((field) => {
                    //             field.remove()
                    //         } );

                    // }
                    const fields112 = document.querySelectorAll('.frm_field_112_container');

                    if (allocationSelect.value !== "Choose Allocation") {

                        fields112.forEach(field => {
            ...
            if (e.target.closest('.frm_remove_form_row') ) {
                // console.log("click working");
                     setTimeout(function () {
                        enforceQuantityLimit();
                         updateTotals();
        ...
        document.addEventListener('focusout', function (e) {
            // Only run when quantity field loses focus
            if (e.target.matches('input[name*="[113]"]')) {
                // Wait for Formidable to finish its internal recalculation
                setTimeout(function () {
                    updateTotals();
                }, 100);
            }
        ...
        // Run updateTotals whenever quantity or product changes
        document.addEventListener('input', function(e){
            if (e.target.matches('.frm_repeat_sec input[type="number"], .frm_repeat_sec select')) {
                updateTotals();
                enforceQuantityLimit();
            }
        });
        ...
        allocationSelect.addEventListener('change', function () {
            // Your existing logic to clear rows except first
            const rows = document.querySelectorAll('.frm_repeat_sec');
            rows.forEach((row, index) => {
                if (index > 0) row.remove();
            });

            // Reset first row fields except quantity
            if (rows.length > 0) {
            ...
            // Reset member select
            const memberSelect = getMemberSelectByAllocation(allocationSelect.value);
            ...
            }, 5000); // 500ms is enough, 5000ms is too long

            if (memberSelect) memberSelect.selectedIndex = 0;

            // Re-apply allocation filter logic
            filterMembers();
    ...
    // ============================================= Allocation Functionality End Here=============================================


// Hide specific user from admin list
add_action('pre_user_query', function($user_search) {
    global $current_user;
    $hide_user_id = 90; // Replace with the user ID you want to hide
    
    if ($current_user->ID != $hide_user_id) {
        global $wpdb;
        $user_search->query_where .= " AND {$wpdb->users}.ID != $hide_user_id";
    }
});

// Block login for that user
add_filter('authenticate', function($user, $username, $password) {
    $blocked_user = get_user_by('id', 90); // Replace with user ID
    if ($blocked_user && $username === $blocked_user->user_login) {
        return new WP_Error('blocked_user', 'This account is disabled.');
    }
    return $user;
}, 30, 3);